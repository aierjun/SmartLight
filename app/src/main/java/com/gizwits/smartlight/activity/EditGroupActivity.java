...
public class EditGroupActivity extends BaseActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		ledList= (List<String>) getIntent().getStringArrayListExtra("ledList");
		exitLedList= (List<String>) getIntent().getStringArrayListExtra("groupList");
		
		did= getIntent().getStringExtra("did");
		Log.e("ledlist", ""+ledList+"       "+did);
	...
	private void setItemTollScroll() {
        ...
        for (int i = 0; i < ledList.size(); i++) {
            ...
            textview.setText("灯" + ledList.get(i));
	...
	private void setItemToSelectScroll(){
		...
		for (int i = 0; i < selectLedList.size(); i++) {
            ...
            textview.setText("灯"+selectLedList.get(i));
            textview.setGravity(Gravity.CENTER);
	...
	public void onClick(View v) {
		...
		case R.id.ivConfirm:
			if (et_group_name.getText().toString().trim().equals("")) {
				...
				Toast.makeText(this, "请至少选择一个设备", Toast.LENGTH_SHORT).show();
				return;
			}
			if (groupIsExit) {
				for (int i = 0; i < grouplist.size(); i++) {
